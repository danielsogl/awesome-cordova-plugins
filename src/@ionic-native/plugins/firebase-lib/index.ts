 ...
 * @name FirebaseLib
 * @description
 * This plugin is a fork from [cordova-plugin-firebase](https://github.com/arnesson/cordova-plugin-firebase) which is not maintened anymore
 * It brings push notifications, analytics, event tracking, crash reporting and more from Google Firebase to your Cordova project! Android and iOS supported (including iOS 10).
 *
 * @usage
 * ```typescript
 ...
 *
 * this.firebaseLib.onTokenRefresh()
 *   .subscribe((token: string) => console.log(`Got a new token ${token}`));
 *
 * ```
 */
@Plugin({
  pluginName: 'FirebaseLib',
...
@Injectable()
export class FirebaseLib extends IonicNativePlugin {
  /**
   * Returns a stable identifier that uniquely identifies the app instance
   * @return {Promise<string>} Instance ID
   */
  @Cordova()
